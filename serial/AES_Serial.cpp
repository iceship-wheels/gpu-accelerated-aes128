...
void AES128_serial::key_expansion()
{
    uint32_t w[4];
    uint32_t w_new[4];
...
void AES128_serial_standard::encrypt_block(uchar input[BLOCK_SIZE], uchar output[BLOCK_SIZE])
{
...
void AES128_serial_standard::aes_1_substituteBytes(uchar state[BLOCK_SIZE]) // (字节代换)
{
...
void AES128_serial_standard::aes_2_permutation(uchar state[BLOCK_SIZE]) // shift row (行移位)
{
...
void AES128_serial_standard::aes_3_multiplication(uchar state[BLOCK_SIZE]) // mix columns (列混合)
{
    uchar state_t[BLOCK_SIZE];
    memcpy(state_t, state, BLOCK_SIZE);
...
void AES128_serial_standard::aes_4_addRoundKey(uchar state[BLOCK_SIZE], uint round) // (轮密钥加)
{
    for (int i = 0; i < BLOCK_SIZE; i++)
    {
...
void AES128_serial_standard::decrypt_block(uchar input[BLOCK_SIZE], uchar output[BLOCK_SIZE])
{
    uchar state[BLOCK_SIZE];
    memcpy(state, input, BLOCK_SIZE);
...
void AES128_serial_standard::aes_inv_1_substituteBytes(uchar state[BLOCK_SIZE])
{
    for (int i = 0; i < BLOCK_SIZE; i++)
    {
...
void AES128_serial_standard::aes_inv_2_permutation(uchar state[BLOCK_SIZE])
{
    uchar state_t[BLOCK_SIZE];
    memcpy(state_t, state, BLOCK_SIZE);
...
void AES128_serial_standard::aes_inv_3_multiplication(uchar state[BLOCK_SIZE])
{
    uchar state_t[BLOCK_SIZE];
    memcpy(state_t, state, BLOCK_SIZE);
    // state = M * Transpose(state_t);
...
AES128_serial_standard::AES128_serial_standard(std::string key)
{
...
void AES128_serial_standard::encrypt(uchar input[], uchar output[], int len)
{
    if (len % BLOCK_SIZE != 0)
    {
...
void AES128_serial_standard::decrypt(uchar input[], uchar output[], int len)
{
    if (len % BLOCK_SIZE != 0)
    {
        std::cerr << "[Error] Input size must be multiple of " << BLOCK_SIZE << " bytes" << std::endl;
